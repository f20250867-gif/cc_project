...
from teams.utils import is_team_owner,is_team_maintainer,is_team_member, is_present_in_team,is_task_assigned_to
...
class TaskCreateView(ActivityCreateUpdateLogMixin, LoginRequiredMixin, UserPassesTestMixin, SuccessMessageMixin, CreateView):
    ...
    success_message = "'%(task_title)s'  task was created successfully"
...
class TaskDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["assigned_member"] = is_task_assigned_to(id=self.object.id, user=self.request.user)
        return context

        
class TaskUpdateView(ActivityCreateUpdateLogMixin, LoginRequiredMixin, UserPassesTestMixin, SuccessMessageMixin,UpdateView):
    ...
    def handle_no_permission(self): 
        messages.error(self.request, "You don't have permission to update a task!")
        return redirect('task-detail', self.kwargs.get('team_pk'), self.kwargs.get('project_pk'),self.kwargs.get('task_pk'))
    
# only owners, maintainers and members to whom the task has been assigned can update task
    def test_func(self):
        project = self.get_project()
        task_id = self.kwargs.get('task_pk') 
        if is_team_owner(self.request.user, project.team) or  is_task_assigned_to(self.request.user, task_id) or is_team_maintainer(self.request.user, project.team):
            return True 
        else:
            return False
        
    success_message = "'%(task_title)s'  task was updated successfully"
...
class TaskDeleteView(ActivityDeleteLogMixin, LoginRequiredMixin, UserPassesTestMixin,SuccessMessageMixin,  DeleteView):
    model = Task
    pk_url_kwarg = 'task_pk'
    ...
    success_message = "'%(task_title)s'  task was deleted successfully"
...
def assign_task(request, team_pk, project_pk, task_pk):
    ...
    if Task.objects.filter(assigned_to=get_user, id=task_pk).exists():
        messages.warning(request, 'user has already been assigned same task')
        return redirect('task-assign', team_pk=team_pk, project_pk=project_pk, task_pk=task_pk)

    task.assigned_to.add(get_user)
    task.save()
    Activity.objects.create(target=task,action='was assigned',user=get_user)
...
class CommentCreateView(ActivityCreateUpdateLogMixin,UserPassesTestMixin, LoginRequiredMixin,CreateView):
    ...
    def test_func(self):
        project = self.get_project()
        if is_present_in_team(self.request.user, project.team):
            return True
        else:
            return False
